...
def main():
    ...
    data['gas'] = calculate_sbdart(data, 'surdcp_gascp', 'fixed')
    data['sza'] = calculate_sbdart(data, 'dcp', 'per_point')
    data['coupled'] = calculate_sbdart(data, 'cp', 'per_point')

    fig, ax = plt.subplots(figsize=(7.2, 5.2))
    cot_fit = np.geomspace(MIN_COT, 76, 200)
    t91_albedo = cot_to_albedo(cot_fit, 'quadrature', sza=54.74)
    ax.plot(cot_fit, t91_albedo, color='#222222', lw=1.8, label=r'T91: $k$=1.00')

    fit_and_plot(ax, data, 'visible', 'Visible', COLORS['visible'], edges)
    fit_and_plot(ax, data, 'shortwave', 'Shortwave', COLORS['shortwave'], edges)
    fit_and_plot(ax, data, 'surface', r'Shortwave, $A_{\mathrm{sfc}}$ Coupled', COLORS['surface'], edges)
    fit_and_plot(ax, data, 'gas', 'Shortwave, Gas Coupled', COLORS['gas'], edges)
    fit_and_plot(ax, data, 'sza', r'Shortwave, SZA$_{\mathrm{1030}}$ Coupled', COLORS['sza'], edges)
