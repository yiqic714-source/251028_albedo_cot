...
def calc_irf_ratios():
    ...
    # Precompute SWdown for all unique lat-month combinations
    ...
    # Calculate IRF for each row
    # IRF_base = (lnnd_o_lnaod / 3) * swdown * log_aod_diff
    irf_base = (merged_df['lnnd_o_lnaod'] / 3.0) * merged_df['swdown'] * merged_df['log_aod_diff']
    
    # Original IRF
    # ret: Ac_ret_orig = 0.13 * cot / (1 + 0.13 * cot)
    cot = merged_df['cot_mod08'].values
    Ac_ret_orig = 0.13 * cot / (1 + 0.13 * cot)
    cf_ret = merged_df['cf_ret_liq_mod08'].values
    irf_ret_orig = irf_base.values * cf_ret * Ac_ret_orig * (1 - Ac_ret_orig)
    
    # msk: Ac_msk from CERES
    Ac_msk = merged_df['cf_liq_ceres'].values  # Using cf_liq_ceres as Ac_msk
    cf_msk = merged_df['cf_liq_ceres'].values
    irf_msk_orig = irf_base.values * cf_msk * Ac_msk * (1 - Ac_msk)
    
    # Corrected IRF for each method/var_type
    results = {
    ...
    for ocean in OCEANS:
        for season in SEASONS:
            mask = (merged_df['ocean'] == ocean) & (merged_df['season'] == season)
            if not mask.any():
                continue
            ...
            b_ret_1030_unc = b_ret_1030 * lnb_ret_1030_unc if not np.isnan(b_ret_1030) else np.nan  # error propagation: db = b * d(lnb)
            ...
            m_msk_1030_unc = 0.0  # m=1 has no uncertainty
            
            # Per-grid-cell IRF ratios
            # ret corrected: Ac_corr = b * cot^k / (1 + b * cot^k)
            # ratio = k * Ac_corr * (1 - Ac_corr) / (Ac_orig * (1 - Ac_orig))
            ...
            # msk 1030: Ac_corr = m * Ac_msk, ratio = k * Ac_corr * (1-Ac_corr) / (Ac * (1-Ac))
    ...
    # Aggregate to ocean level (area-weighted across seasons)
    # For simplicity, average across seasons
    ocean_results = {}
