...
if __name__ == "__main__":
    ...
    fig, axes = plt.subplots(3, 1, figsize=(5, 9.5))
    ...
    ax1.text(-0.01, 1.07, r'$\mathbf{(a)}$', transform=ax1.transAxes, 
         fontsize=16, va='center', ha='left')
    ax1.text(0.5, 1.07, 'Full Granule', transform=ax1.transAxes, 
    ...
    ax2.text(-0.01, 1.07, r'$\mathbf{(b)}$', transform=ax2.transAxes, 
         fontsize=16, va='center', ha='left')
    ax2.text(0.5, 1.07, 'Sampled Grid', transform=ax2.transAxes, 
            fontsize=13, va='center', ha='center')
    ...
    # (c) Retrieval weight + 橙色点标注COT/反照率
    ax3 = axes[2]
    colors_white2blue = [
        (1.0, 1.0, 1.0, 1.0),  # 白
    ...
    ax3.text(-0.01, 1.07, r'$\mathbf{(c)}$', transform=ax3.transAxes, 
         fontsize=16, va='center', ha='left')
    ax3.text(0.14, 1.07, 'Fields of Successful Retrievals', transform=ax3.transAxes, 
    ...
    # 标注每个橙色中心点，格式为 "(COT, albedo)"，小字体、无框
    if center_latlon is not None and len(center_latlon) > 0:
    ...
    ax3.text(0.01, 0.68, r'(COT$_{\mathrm{ret}}, A_{\mathrm{c,ret}})$:', transform=ax3.transAxes, fontsize=10, va='top', ha='left')
    # Customize longitude ticks
