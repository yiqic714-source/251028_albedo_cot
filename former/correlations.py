...
OUTPUT_FIG_COMBINED = Path("/home/chenyiqi/251028_albedo_cot/figs/scatter_correlations_1x2.png")
...
def plot_scatter_with_fit(points_df, ax):
	...
	ax.set_title("(a)", loc="left", fontsize=15, fontweight="bold")
...
def plot_cotdisp_vs_kcp_minus_kret(points_df, ax):
	...
	ax.set_title("(b)", loc="left", fontsize=15, fontweight="bold")


def main():
	points_df = build_points()
	fig, axes = plt.subplots(1, 2, figsize=(10, 4.7))
	plot_cotdisp_vs_kcp_minus_kret(points_df, axes[0])
	plot_scatter_with_fit(points_df, axes[1])

	plt.tight_layout()
	OUTPUT_FIG_COMBINED.parent.mkdir(parents=True, exist_ok=True)
	fig.savefig(OUTPUT_FIG_COMBINED, dpi=300, bbox_inches="tight")
	plt.close(fig)
	print(f"Saved combined 1x2 scatter figure to: {OUTPUT_FIG_COMBINED}")
