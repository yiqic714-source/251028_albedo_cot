...
def main():
    ...
    grid_weights = np.cos(np.deg2rad(lat))[:, np.newaxis]
    for name, regions in OCEANS.items():
        ...
        region_area_weights[name] = np.sum(np.where(mask, grid_weights, 0.0))
    ...
    for method in ("ret", "msk"):
        method_values = bar_diff[bar_diff["method"] == method].set_index("ocean")["bar_diff"]
        valid = np.array([
            np.isfinite(method_values.get(name, np.nan))
            and np.isfinite(region_area_weights[name])
            and region_area_weights[name] > 0
            for name in OCEANS
        ])
        names = list(OCEANS)
        values = np.array([method_values.get(name, np.nan) for name in names], dtype=float)
        weights = np.array([region_area_weights[name] for name in names], dtype=float)
        combined_irf_aci[method] = np.sum(values[valid] * weights[valid]) / np.sum(weights[valid])
    ...
    draw_region_map(
        ...
        r'$\mathrm{IRF}_\mathrm{aci}$ Overestimation / $\mathrm{ERF}_\mathrm{CO2}$ (Cloud-Retrieval)', 1,
    )
    draw_region_map(
        axes[2], ratio_by_method["msk"], ratio_norm, ratio_cmap,
        r'$\mathrm{IRF}_\mathrm{aci}$ Overestimation / $\mathrm{ERF}_\mathrm{CO2}$ (Cloud-Mask)', 2,
    )

    fig.colorbar(
        ScalarMappable(norm=erf_norm, cmap=erf_cmap), ax=axes[0],
        orientation="vertical", shrink=0.7, aspect=15, pad=0.04,
        boundaries=erf_boundaries, spacing="proportional",
    )
    fig.colorbar(
        ScalarMappable(norm=ratio_norm, cmap=ratio_cmap), ax=axes[1:],
        orientation="vertical", shrink=0.7, aspect=30, pad=0.04,
    ...
    print("All ocean regions combined area-weighted IRF_aci (W m-2):")
    print(f"  Ret: {combined_irf_aci['ret']:.6f}")
    print(f"  Msk: {combined_irf_aci['msk']:.6f}")
