...
def _compute_mean_curve(cot, albedo):
    ...
    # Define x-bin edges from 0.75 to 3.5 with step 0.25
    x_bin_edges = np.arange(0.75, 3.75, 0.4)
...
def plot_figure(icon_style='nature'):
    ...
    for col_idx, method_key in enumerate(method_keys):
        ...
        ax0.set_xlim([0.75, 3.5])
        ax0.set_ylim([-1.6, 1.2])
        ...
        if col_idx == 0:
            ax0.legend(fontsize=11, loc='upper left', ncol=2)
        else:
    ...
    output_fig_path = 'figs/fig2_fittings_by_ocean_and_aod.png'
